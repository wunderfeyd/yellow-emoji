...
class YellowEmojis
{
	...
	function emojifyText($text)
	{
		while(true)
		{
			$elementFound = preg_match("/\:[^\s\!\?]*?\:/s", $text, $matches, PREG_OFFSET_CAPTURE, $offsetBytes);
			$element = $matches[0][0];
			$elementOffsetBytes = $elementFound ? $matches[0][1] : strlenb($text);
			$string = substrb($text, $offsetBytes, $elementOffsetBytes - $offsetBytes);
			$output .= $string;
			if(!$elementFound) break;
			$image = $this->emojis[strtolower($element)];
			if($image!=null)
			{
				$output .= "<img class=\"emoji\" title=\"".$element."\" alt=\"".$element."\" src=\"".$this->yellow->config->get("emojisImageLocation").$image.".png\"/>";
			} else {
				$output .= $element;
			}
			$offsetBytes = $elementOffsetBytes + strlenb($element);
